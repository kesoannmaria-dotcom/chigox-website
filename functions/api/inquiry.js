      ...
      <table class="email-shell" role="presentation" width="640" cellspacing="0" cellpadding="0" border="0" style="width:640px;max-width:640px;background:#fff;border:1px solid #d9e5ed;border-radius:12px;overflow:hidden;">
        ...
        <tr><td class="email-pad" style="padding:30px;">
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="width:100%;margin-top:18px;border-collapse:separate;border-spacing:0;border:1px solid #d9e5ed;border-radius:8px;overflow:hidden;">
            ...
            ${fieldRow("UTM content", inquiry.utmContent, { fallback: "Not captured" })}
            ${fieldRow("UTM term", inquiry.utmTerm, { fallback: "Not captured" })}
            ${fieldRow("Source", inquiry.source, { fallback: "Unknown" })}
            ${fieldRow("Medium", inquiry.medium, { fallback: "Unknown" })}
            ${fieldRow("Channel", inquiry.channel, { fallback: "Direct / Unknown" })}
            ${fieldRow("SEO likely", inquiry.seoLikely === "yes" ? "Yes" : "No")}
            ${fieldRow("Keyword", inquiry.keyword, { fallback: "Not available" })}
...
export function renderInquiryText(inquiry) {
  return [
    ...
    `UTM content: ${inquiry.utmContent || "Not captured"}`,
    `UTM term: ${inquiry.utmTerm || "Not captured"}`,
    `Source: ${inquiry.source || "Unknown"}`,
    `Medium: ${inquiry.medium || "Unknown"}`,
    `Channel: ${inquiry.channel || "Direct / Unknown"}`,
    `SEO likely: ${inquiry.seoLikely === "yes" ? "Yes" : "No"}`,
    `Keyword: ${inquiry.keyword || "Not available"}`,
...
function validateBody(body, requestUrl) {
  ...
  return {
    ...
    utmContent: cleanText(body.utm_content || "", FIELD_LIMITS.attribution),
    utmTerm: cleanText(body.utm_term || "", FIELD_LIMITS.attribution),
  };
}

function normalizedReferrerHost(referrer) {
  if (!referrer) return "";
  try {
...
function isMetaHost(hostname) {
  return hostname === "facebook.com" || hostname.endsWith(".facebook.com") || hostname === "instagram.com" || hostname.endsWith(".instagram.com");
}

function isOtherSearchHost(hostname) {
  return ["bing.", "yahoo.", "duckduckgo.", "baidu.", "yandex.", "ecosia.", "search.brave.com"]
    .some((candidate) => hostname === candidate.slice(0, -1) || hostname.startsWith(candidate));
}
...
export function classifyAttribution(input) {
  ...
  if (utmSource || utmMedium || input.utmCampaign || input.utmContent || input.utmTerm) {
    if (["facebook", "fb", "instagram", "ig", "meta"].includes(utmSource)) {
      return {
        source: utmSource === "ig" ? "instagram" : utmSource === "fb" ? "facebook" : utmSource,
    ...
    return { source: utmSource || "utm", medium: utmMedium || "unknown", channel: "UTM Campaign", seoLikely: "no", keyword: "" };
  }

  const host = normalizedReferrerHost(input.referrer);
  if (isGoogleHost(host)) {
    return { source: "google", medium: "organic", channel: "Google Organic", seoLikely: "yes", keyword: "Keyword unavailable" };
  }
  if (isMetaHost(host)) {
    return { source: host.includes("instagram") ? "instagram" : "facebook", medium: "referral", channel: "Facebook / Meta", seoLikely: "no", keyword: "" };
  }
  if (isOtherSearchHost(host)) {
    return { source: host, medium: "organic", channel: "Organic Search", seoLikely: "yes", keyword: "Keyword unavailable" };
  }
  if (host) return { source: host, medium: "referral", channel: "Referral", seoLikely: "no", keyword: "" };
  return { source: "unknown", medium: "unknown", channel: "Direct / Unknown", seoLikely: "no", keyword: "" };
}
...
async function persistInquiry(env, inquiry, tokenHash) {
  const insertInquiry = env.INQUIRY_DB.prepare(`
    INSERT INTO inquiries (
      ...
      utm_campaign, utm_content, utm_term, source, medium, channel, seo_likely,
      keyword, visitor_country, turnstile_hostname, turnstile_action,
      notification_status
    ) VALUES (
