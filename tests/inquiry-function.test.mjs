...
function persistedInquiry(db) {
  const insert = db.savedStatements.find((statement) => statement.sql.includes("INSERT INTO inquiries"));
  const columns = [
    "inquiryId", "submissionId", "createdAt", "name", "email", "company", "product", "message",
    "pagePath", "pageLanguage", "landingPage", "referrer", "utmSource", "utmMedium", "utmCampaign",
    "utmContent", "utmTerm", "source", "medium", "channel", "seoLikely", "keyword", "visitorCountry",
    "turnstileHostname", "turnstileAction",
  ];
  return Object.fromEntries(columns.map((column, index) => [column, insert.parameters[index]]));
}
...
test("valid inquiry is verified, persisted, then emailed as an HTML table", async () => {
  ...
  assert.match(resendPayload.html, /UTM content/);
  assert.match(resendPayload.html, /Channel/);
  ...
  assert.match(resendPayload.text, /UTM content: Not captured/);
  assert.match(resendPayload.text, /Channel: UTM Campaign/);
...
test("attribution classification is conservative and explainable", () => {
  const blank = { utmSource: "", utmMedium: "", utmCampaign: "", utmContent: "", utmTerm: "", referrer: "" };
  assert.deepEqual(classifyAttribution(blank), {
    source: "unknown", medium: "unknown", channel: "Direct / Unknown", seoLikely: "no", keyword: "",
  });
  assert.deepEqual(classifyAttribution({ ...blank, utmSource: "facebook", utmMedium: "social", utmCampaign: "test" }), {
    source: "facebook", medium: "social", channel: "Facebook / Meta", seoLikely: "no", keyword: "",
  });
  assert.deepEqual(classifyAttribution({ ...blank, referrer: "https://www.google.co.uk/search" }), {
    source: "google", medium: "organic", channel: "Google Organic", seoLikely: "yes", keyword: "Keyword unavailable",
  });
  assert.deepEqual(classifyAttribution({ ...blank, referrer: "https://l.facebook.com/" }), {
    source: "facebook", medium: "referral", channel: "Facebook / Meta", seoLikely: "no", keyword: "",
  });
  assert.deepEqual(classifyAttribution({ ...blank, referrer: "https://www.bing.com/search?q=ultrasound" }), {
    source: "bing.com", medium: "organic", channel: "Organic Search", seoLikely: "yes", keyword: "Keyword unavailable",
  });
  assert.deepEqual(classifyAttribution({ ...blank, referrer: "https://example-distributor.com/catalog" }), {
    source: "example-distributor.com", medium: "referral", channel: "Referral", seoLikely: "no", keyword: "",
  });
});

test("D1 persists source attribution for Direct, UTM and referral submissions", async (t) => {
  const cases = [
    {
      name: "Direct / Unknown",
      body: makeBody({ utm_source: "", utm_medium: "", utm_campaign: "", referrer: "" }),
      expected: { source: "unknown", medium: "unknown", channel: "Direct / Unknown", seoLikely: "no", keyword: "" },
    },
    {
      name: "Facebook / Meta UTM",
      body: makeBody({ utm_source: "facebook", utm_medium: "social", utm_campaign: "test", utm_content: "preview", utm_term: "probe" }),
      expected: { source: "facebook", medium: "social", channel: "Facebook / Meta", seoLikely: "no", keyword: "" },
    },
    {
      name: "Referral",
      body: makeBody({ utm_source: "", utm_medium: "", utm_campaign: "", referrer: "https://partner.example.org/product" }),
      expected: { source: "partner.example.org", medium: "referral", channel: "Referral", seoLikely: "no", keyword: "" },
    },
  ];
  for (const scenario of cases) {
    await t.test(scenario.name, async () => {
      ...
      const response = await handleInquiryPost(
        { request: makeRequest(scenario.body), env: makeEnv(db, { RESEND_API_KEY: undefined }) },
        { ...dependencies, fetchImpl: mocked.fetchImpl },
      );
      assert.equal(response.status, 201);
      const persisted = persistedInquiry(db);
      assert.deepEqual(
        { source: persisted.source, medium: persisted.medium, channel: persisted.channel, seoLikely: persisted.seoLikely, keyword: persisted.keyword },
        scenario.expected,
      );
      assert.equal(persisted.visitorCountry, "CO");
      assert.equal(persisted.utmContent, scenario.body.utm_content || "");
      assert.equal(persisted.utmTerm, scenario.body.utm_term || "");
    });
  }
});
...
test("email template escapes all customer-supplied HTML", () => {
  ...
  assert.match(html, /UTM content/);
  assert.match(html, /Keyword unavailable/);
});
